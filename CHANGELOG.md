...
- 페이지 외에 ajax·GET·POST 요청이 있으면 기본 페이지 로그가 사라지고 부가 요청 로그만 남던 문제 수정 — 하나의 내비게이션을 커밋 릴레이(`NAV_COMMITTED`)와 `devtools.network.onNavigated` 가 각각 정리하면서, 뒤늦은 두 번째 정리가 "이동한 URL과 일치하는 마지막 요청"을 새 페이지의 경계로 잡아 페이지와 같은 URL로 보낸 ajax 앞의 문서 요청이 잘려나갔다. 이미 정리한 내비게이션은 다시 정리하지 않고, 경계 탐색도 가장 이른 일치로 바꿔 같은 URL 요청이 문서 항목을 잘라내지 못하게 했다
- `history.pushState`·hash 이동에서 요청 목록이 비워지던 문제 수정 — `performance.timeOrigin` 으로 문서 교체 여부를 확인해 same-document 이동에서는 목록을 유지한다
- 같은 URL을 빠르게 다시 로드하면 이전 로드의 요청이 목록에 남던 문제 수정 — 현재 페이지의 문서 요청을 경계 후보에서 제외
- 언더스코어가 든 호스트명을 와일드카드로 지정하면(`*.nova_builder.zzz`) "잘못된 입력값이 있습니다" 로 저장이 거부되던 문제 수정 — 와일드카드 키 검증 문자 집합에 `_` 가 빠져 있었다. 정확 호스트명(`nova_builder.zzz`)은 URL 파서를 거쳐 이미 허용되고 있어 두 경로의 판정이 달랐다

### 테스트

- 내비게이션 정리 로직 회귀 테스트 추가 (`node test/nav-clear.test.js`) — devtools 스크립트를 vm 샌드박스에 올려 브라우저별 이벤트 순서(커밋 릴레이 유무, 지연 `onNavigated`, same-document 이동, preserve log)를 재현해 검증
- 호스트 키 정규화·매칭 회귀 테스트 추가 (`node test/host-key.test.js`) — 언더스코어 호스트명, 전체 허용 패턴 거부, 패턴 매칭 범위 검증
